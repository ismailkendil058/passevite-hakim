...
async function test() {
    const { data, error } = await supabase.from('labo_orders').select('*').limit(1);
    if (error) {
        console.error("Error:", error);
        // If error, try to list columns by selecting something else or checking RPC if available
        // But usually we can just see the error if it's about a specific column.
    } else {
