...
async function test() {
    const { data: res, error } = await supabase.rpc('get_table_info', { t_name: 'labo_orders' });
    if (error) {
        console.log("RPC Error (expected if not exists):", error);
        // Fallback: try to guess columns
        const cols = ['date', 'order_date', 'prescription_date', 'created_at', 'labo_date'];
