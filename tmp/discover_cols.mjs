...
async function test() {
    const { data, error } = await supabase.from('labo_orders').insert({}).select();
    if (error) {
        console.error("Error inserting empty:", error);
    } else {
        console.log("Inserted! Columns:", Object.keys(data[0]));
        // Clean up
        await supabase.from('labo_orders').delete().eq('id', data[0].id);
    }
}
test();
